...
def eval(dpath, output_path):
    ...
    # accuracy, f1, auc
    scores["accuracy"] = metrics.accuracy_score(y_true=df.label, y_pred=df.pred)
    ...
    for task in tasks:
        ...
        if ("Polish" in dpath or "Italian" in dpath) and task in [
            "country",
            "ethnicity",
        ]:
            continue
...
def data_iter(datap, batch_size=64, if_shuffle=True, if_sample=False):
    ...
    if if_sample:
        ...
        for label_tmp in label_count:
            ...
            sample_indices = np.random.choice(
                list(range(len(data["y"]))), size=sample_num
            )
    ...
    class_weights = compute_class_weight(
        class_weight="balanced", classes=classes, y=data["y"]
    )
