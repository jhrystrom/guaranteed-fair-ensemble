...
cfg = DatasetConfig(
    name="ham10000",
    csv_relpath=Path("HAM10000_metadata.csv"),
    img_relpath=Path("HAM10000_images"),
    path_col="image_id",
    target_col="dx",
    protected_col="age",
...
def load_and_clean_data(
    ...
    Returns:
        Cleaned DataFrame
    """
    output_path = data_dir / "ham10000_cleaned.csv"
    ...
    df_parsed = df_full.with_columns(
        pl.col(cfg.target_col).is_in({"akiec", "mel"}).cast(pl.Int8),
        pl.when(pl.col(cfg.protected_col) < 40)
        .then(0)
        .when(pl.col(cfg.protected_col).is_between(40, 60))
        .then(1)
        .otherwise(2)
        .alias(cfg.protected_col),
    )
...
def load_images(image_paths: list[Path]) -> dict[str, Tensor]:
    ...
    pickle_path = data_dir / "ham10000_images.pkl"
